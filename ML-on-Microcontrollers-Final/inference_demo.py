# Author: Keyshav Mor <keymor@student.ethz.ch> (ETH Zurich) 
...
for class_label in range(10):
    ...
    # Randomly select 1 indices
    selected_indices = random.sample(list(class_indices), 1)
...
if __name__ == '__main__':
    ...
    ser = serial.Serial(port='/dev/tty.usbmodem14103', baudrate=115200, 
timeout=3)
    ...
    correct_count = 0
    cumulative_inference_count = 0
    sys_clock_frequency = 80000000
    inference_count = 100
    test_len = 10
    ...
    for image, label in zip(x, y):
        ...
        time.sleep(1)
        cycle_count = ser.read(4)
        cycle_count = np.frombuffer(cycle_count, dtype=np.uint8)
        # Reconstruct the 32-bit integer
        reconstructedValue = int.from_bytes(cycle_count, byteorder='little')
        # Print the reconstructed integer
        print("Cycle Count:", reconstructedValue)
        cumulative_inference_count += reconstructedValue
        time.sleep(1)
        pred = ser.read(10)
...
average_cycle_count_inference = cumulative_inference_count/test_len
average_inference_time = average_cycle_count_inference/sys_clock_frequency
print("Average inference time (s):", average_inference_time)
print("Correctly classified images count:", correct_count)
print("Accuracy %:", (correct_count/test_len)*100)
